{-# language TemplateHaskell, MultiParamTypeClasses, TypeFamilies, ScopedTypeVariables, PartialTypeSignatures, TypeApplications #-}
...
instance Typeable domain => Request (GetLibrary domain) where
  type Req (GetLibrary domain) = (Int,Txt)
...
api = WS.api msgs reqs
  where
    msgs = WS.none
    reqs = getLibrary @domain <:> upload @domain <:> delete @domain <:> WS.none